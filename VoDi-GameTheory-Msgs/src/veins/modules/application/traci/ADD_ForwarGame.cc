...
Define_Module(ADD_ForwarGame)

void ADD_ForwarGame::initialize(int stage)
{
...
void ADD_ForwarGame::receiveSignal(cComponent *source, simsignal_t signalID, cComponent::cObject *obj)
{
    Enter_Method_Silent();
...
void ADD_ForwarGame::onBeacon(WaveShortMessage *wsm1)
{
    /*SCF*/
    ...
    double alfa_j=wsm->getKjUj();
    (alfa_j<=0)?alfa_j=1:alfa_j=alfa_j*1;

    vectorKjUj[wsm->getCarId()]=(alfa_j);


    vectorKiUi[simTime()]=availabilityA(d,abeLocalB);
...
double ADD_ForwarGame::availabilityA( double d, double ab)
{
    return 0.6*d+0.4*ab;
}


double ADD_ForwarGame::betaCalculation(int k, int n, double alfa)
{
...
void ADD_ForwarGame::onData(WaveShortMessage *wsm)
{
...
/***************************************************************************************************************************************/
   ...
   // double difDistance=abs(distanceThreshold-distanceFromPreviousNode);
    ...
    for( map<int, double >::iterator it = vectorKjUj.begin(); it != vectorKjUj.end(); ++it ) {
     ...
     double si=(1-S_i)*betaCalculation(4,3,(double)availabilityA(d,abeLocalD));
    ...
    (si)>0 ? probabilidadEnvio=si:probabilidadEnvio=0;
...
void ADD_ForwarGame::handlePositionUpdate(cComponent::cObject *obj)
{
...
void ADD_ForwarGame::handleSelfMsg(cMessage *msg)
{
...
void ADD_ForwarGame::forwardMessage(double p)
{
/*
...
void ADD_ForwarGame::sendMessage(std::string blockedRoadId)
{
    sentMessage = true;
...
WaveShortMessage*  ADD_ForwarGame::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, int serial,
...
double ADD_ForwarGame:: kiUi(WaveShortMessage* wsm ) {
    double dsr=(mobility->getPositionAt(simTime()).distance(wsm->getSourcePos()));
    double distanceFromPreviousNode=mobility->getPositionAt(simTime()).distance(wsm->getSenderPos());
    double d=distanceFactorCalc(dsr,calcDistJoin(wsm),distanceThreshold,vehicleOnJunction());
    ...
    return (double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lqD);
...
double ADD_ForwarGame::getLinkQuality(double chq, double cp, double sq ){
...
double ADD_ForwarGame::calAgeFactor(double timeElapsed)
{
    return pow(0.99,timeElapsed);
}


bool ADD_ForwarGame::hostIsClosestToJunction(string junctionId)
{
...
bool ADD_ForwarGame::vehicleOnJunction()
{
...
string ADD_ForwarGame::getIdJunction()
{
    // check to see if this host is near an intersection
...
double ADD_ForwarGame::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
double ADD_ForwarGame::getChannelQuality() {
...
double ADD_ForwarGame::getSignalQuality(WaveShortMessage* wsm) {
...
double ADD_ForwarGame::getCollisionProbability() {
...
double ADD_ForwarGame::calcAbe(WaveShortMessage* wsm, double cpb)
{
...
double ADD_ForwarGame::calcDistJoin(WaveShortMessage* wsm)
{
...
double ADD_ForwarGame::getMin(std::map<std::string, double> mymap)
{
...
double ADD_ForwarGame::beaconingMain(WaveShortMessage* wsm, int neighbors, double distanceFromPreviousNode, double distanceToIntersection )
{
...
double ADD_ForwarGame::beaconingChannel(double neighbors, double wc, WaveShortMessage* wsm)
{
...
double ADD_ForwarGame::beaconingPriority( double dEv, double dInt, WaveShortMessage* wsm)
{
...
double ADD_ForwarGame::calBeqAsimetricoMult(double ki, double ui, double kjuj, double n)
{
    return (ui/ki)*(pow(kjuj,1/(n-1)));
}

double ADD_ForwarGame::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double ADD_ForwarGame::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
    return pow(10,10-(0*ageFactor+6*distanceFactor+4*linkQualityFactor));
}


long double ADD_ForwarGame::vodM(vector<long double> u){
...
int ADD_ForwarGame::forwardElim(double** mat, int N, int M){
...
vector<long double> ADD_ForwarGame::backSub(double** mat, int N, int M){
...
vector<long double>  ADD_ForwarGame::gaussianElimination(double** mat, int N, int M){
...
void  ADD_ForwarGame::swap_row(double** mat, int N, int M, int i, int j){
...
void ADD_ForwarGame::print(double** mat, int N, int M){
...
double ADD_ForwarGame::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
