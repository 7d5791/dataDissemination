...
#include "veins/modules/application/traci/metricsBySecond.h"
...
const simsignalwrap_t metricsBySecond::parkingStateChangedSignal = simsignalwrap_t(TRACI_SIGNAL_PARKING_CHANGE_NAME);
const double timeSlot = 0.000013;

Define_Module(metricsBySecond);



void metricsBySecond::initialize(int stage) {
 ...
 */
void metricsBySecond::LogToFile(metrics m,WaveShortMessage* msg) {
        ...
        if (std::string(wsm->getName()) == "beacon") {
            of.open(
                      string("./results/")
                              ...
                              + string("-beacon-msg.txt"), ios_base::app);
              if (of.is_open()) {

                           of << simTime() << " : "
                              << "AF=  " << " " << m.getAgeFactor() << " " << " "
                              << "LQF= " << " " <<m.getLinkQualityFactor()<< " " << " "
                              << "ChQ:" << " "<< m.getChannelQuality()<< " " << " "
                              << "SigQ:" << " "<< m.getSignalQuality()<< " " << " "
                              << "ColPr:" << " "<< m.getCollisionProbability()<< " " << " "
                              << "DF=" << " "<< m.getdistanceFactor()<< " " << " "
                              << "ABE=" << " "<< m.getBandwidth()<<  " " << " "
                              << "Nodes=" << " "<< m.getNumberOfNodes()<<endl;
              }
            ...
            ofstream fc( string("./results/")
                    + getParentModule()->getParentModule()->getFullName()
                    + string("-") + getParentModule()->getFullName()
                    + string("-warning-msg.txt"), ios_base::app);

            fc << simTime() << " : " <<"AF= " << " " << m.getAgeFactor() << " " << " "
                    << "LQF= " <<m.getLinkQualityFactor()<< " " << " "
                    << "ChQ:" << " "<< m.getChannelQuality()<< " " << " "
                    << "SigQ:" << " "<< m.getSignalQuality()<< " " << " "
                    << "ColPr:" << " "<< m.getCollisionProbability()<< " " << " "
...
metricsBySecond::metricsBySecond()
{
...
double metricsBySecond::betaCalculation(int k, int n, double alfa)
{
...
void metricsBySecond::onBeacon(WaveShortMessage* wsm) {
    receivedBeacons++;
    metrics msgBeacon;
    ...
    double abeLocal=calcAbe(wsm,cpB);
    ...
    if (ListBeacon.SearchBeacon(b)){
            ListBeacon.UpdateBeacon(wsm->getCarId(),wsm->getArrivalTime(), wsm->getCreationTime(), wsm->getCarId(),wsm->getSpeed(), wsm->getVecX(),
                    wsm->getVecY(), wsm->getVecZ(), calcDistJoin(wsm),dsr, wsm->getTimeIdleChannel(),abeLocal, wsm->getAngleRad());
    }

    else{
            ListBeacon.AddBeacon(wsm->getCarId(),wsm->getArrivalTime(),  wsm->getCreationTime(), wsm->getCarId(),wsm->getSpeed(), wsm->getVecX(), wsm->getVecY(), wsm->getVecZ(),calcDistJoin(wsm), dsr,wsm->getTimeIdleChannel(),abeLocal,wsm->getAngleRad());
    }
   ...
   //calc average of linkQualityFactor map for Thello
    ...
    double r11=0;
    if(lqB>0) r11=abs((lqB-averageLQ)/lqB);
    r1.push_back(r11);
...
////////////////////////////////////////////////////////////////////
  ...
  // EV<<"calUtility: "<<calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lqB)<<endl;

   EV<<"calBeq: r=1 :"<<calBeq(1,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lqB),1,(double)ListBeacon.CounterBeacons())<<endl;

   EV<<"calBeq: r=0 :"<<calBeq(1,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lqB),0,(double)ListBeacon.CounterBeacons())<<endl;

    //EV<<"calBeq: r=0 :"<<calBeq(1,0.5,0,(double)ListBeacon.CounterBeacons())<<endl;

///////////////////////////////////////////////////////////////////
//Forwarding Game
   //vectorBeta[wsm->getCarId()]=betaCalculation(4,3,(double)calUtility(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()),d,lqB));
   vectorBeta[wsm->getCarId()]=betaCalculation(4,3,abeLocal);

  // double t= simTime().dbl()-wsm->getCreationTime().dbl();
  // double base=0.9998;
   /*Log to file */
   msgBeacon.setAgeFactor(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()));
   msgBeacon.setLinkQualityFactor(lqB);
   msgBeacon.setdistanceFactor(d);
   msgBeacon.setBandwidth(abeLocal);

   msgBeacon.setLinkQualityFactor(lqB);
   msgBeacon.setChannelQuality(chqB);

   msgBeacon.setNumberOfNodes(ListBeacon.CounterBeacons());
   msgBeacon.setSignalQuality(sqB);
   msgBeacon.setCollisionProbability(cpB);
   LogToFile(msgBeacon,wsm);



}

double metricsBySecond::forwardingGame(int N,int M, vector<long double> beta){
       double** mat;
       mat = new double*[N];
       for(int i = 0; i < N; ++i)
           mat[i] = new double[M];
...
double metricsBySecond::getLinkQuality(double chq, double cp, double sq ){
...
double metricsBySecond::calBeq(double k, double u, double r, double n){

    return pow((k/((u*(1+r*(n-1))))),(1/(n-1)));
}


double metricsBySecond::calUtility(double ageFactor, double distanceFactor, double linkQualityFactor)
{
...
double metricsBySecond::calAgeFactor(double timeElapsed)
{
...
double metricsBySecond::calS(updatedWeightsMetrics vectorR, double r)
{
   double s=0;
   double max=0;
...
double metricsBySecond::calAverage(vectorMetrics metric, simtime_t tHello)
{
...
void metricsBySecond::onData(WaveShortMessage* wsm) {
    receivedData++;
    metrics msgWarning;
    ...
    double abeLocal=calcAbe(wsm,cpD);
    ...
    double lq=getLinkQuality(chqD,cpD,sqD);
...
///////#### Change availability by bandwidth
     ...
     double si=(1-S_i)*betaCalculation(4,3,abeLocal);
     ...
     /*Log to file */
     msgWarning.setAgeFactor(calAgeFactor(simTime().dbl()-wsm->getCreationTime().dbl()));
     msgWarning.setLinkQualityFactor(lq);
     msgWarning.setdistanceFactor(d);
     msgWarning.setBandwidth(abeLocal);


     msgWarning.setChannelQuality(chqD);

     msgWarning.setNumberOfNodes(nodes);
     msgWarning.setSignalQuality(sqD);
     msgWarning.setCollisionProbability(cpD);
     LogToFile(msgWarning,wsm);
...
void metricsBySecond::sendMessage(std::string blockedRoadId) {
    sentMessage = true;
...
void metricsBySecond::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj) {
    Enter_Method_Silent();
...
void metricsBySecond::handleParkingUpdate(cObject* obj) {
...
void metricsBySecond::handlePositionUpdate(cObject* obj) {
...
void metricsBySecond::sendWSM(WaveShortMessage* wsm) {
	if (isParking && !sendWhileParking) return;
	sendDelayedDown(wsm,individualOffset);
}

void metricsBySecond::handleSelfMsg(cMessage *msg)
{
...
void metricsBySecond::forwardMessage(double p)
{
/*
...
bool metricsBySecond::hostIsClosestToJunction(string junctionId)
{
...
bool metricsBySecond::vehicleOnJunction()
{
...
string metricsBySecond::getIdJunction()
{
    // check to see if this host is near an intersection
...
double metricsBySecond::getMin(std::map<std::string, double> mymap)
{
...
double metricsBySecond::distanceFactorCalc(double d_sr, double d_ri, double r_max, bool  r_vehicleOnIntersection)
{
...
void metricsBySecond::printPhLayer(WaveShortMessage* wsm) {
...
void metricsBySecond::printMacLayer(WaveShortMessage* wsm) {
...
double metricsBySecond::getChannelQuality() {
...
double metricsBySecond::getSignalQuality(WaveShortMessage* wsm) {
...
double metricsBySecond::getCollisionProbability() {
...
void metricsBySecond::printTracyLayer(WaveShortMessage* wsm) {
...
double metricsBySecond::calcAbe(WaveShortMessage* wsm, double cpb)
{
...
double metricsBySecond::calcDistJoin(WaveShortMessage* wsm)
{
...
int metricsBySecond::forwardElim(double** mat, int N, int M){
...
vector<long double> metricsBySecond::backSub(double** mat, int N, int M){
...
vector<long double>  metricsBySecond::gaussianElimination(double** mat, int N, int M){
...
void  metricsBySecond::swap_row(double** mat, int N, int M, int i, int j){
...
void metricsBySecond::print(double** mat, int N, int M){
